...
function saveFile($file,$destinationPath){
    ...
    $coverImg=Image::make($file->getRealPath())->resize(400, null, function ($constraint) {
...
if (!function_exists('uploadFile')) {
    function uploadFile($file, $location, $quality = 85)
    {
        ...
        if (in_array($extension, ['png', 'jpg', 'jpeg'])) {
            $coverImg = Image::make($fileWithExt->getRealPath())->resize(400, null, function ($constraint) {
                $constraint->aspectRatio();
                $constraint->upsize();
            });
            $coverImg->orientate();
